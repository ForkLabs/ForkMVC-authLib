...
class Authlib extends Libraries {
	...
	function is_authenticated() {
		...
		if(!$this->token) {
			...
			$sql = 'SELECT * FROM '.$app_name.'_sessions WHERE session_token = "'.$this->token.'"';
	...
	function authenticate($login_details) {
		...
		if(is_array($login_details)) {
			...
			$sql = 'SELECT * FROM '.$app_name.'_users WHERE username LIKE "'.$username.'" AND user_password = "'.$password.'"';
			$sth = $this->handler->query($sql);
			$result = $sth->fetch(PDO::FETCH_ASSOC);

			if($result) {
				# Lets create the session on the database table
				$sql = 'INSERT INTO '.$app_name.'_sessions (
						session_token,
						session_user,
						session_ip,
						session_useragent,
				...
				$return_array['error_info'] = $sth->errorInfo();
			}
		} else {
			$return_array['status'] = 0;
	...
	function destroy_session() {
		$token_id = $this->token;
		$expiry = time() - 100;
		$sql = 'UPDATE '.$app_name.'_sessions SET session_time_expiration = 1 WHERE session_token = "'.$token_id.'"';
		$sth = $this->handler->query($sql);
